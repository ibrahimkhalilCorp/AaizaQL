...
aqlix.cli
──────────
Command-line interface for aqlix.
...
Usage examples::
    ...
    aqlix query --db sqlite:///mydata.db --llm groq -q "How many users signed up last month?"

    # Ingest schema only (useful for CI / pre-caching)
    aqlix ingest --db sqlite:///mydata.db
...
def _require(pkg: str, install: str) -> None:
    ...
    try:
        ...
        print(f"[aqlix] Missing dependency: {pkg}")
...
def cmd_ingest(args: object) -> None:
    ...
    print(f"[aqlix] Connecting to {args.db} …")  # type: ignore[attr-defined]
    engine = QueryEngine(
    ...
    print(f"[aqlix] Schema ingested: {count} table chunk(s) stored.")
    engine.close()
...
def cmd_query(args: object) -> None:
    ...
    print(f"[aqlix] Connecting with LLM={args.llm} DB={args.db} …")  # type: ignore[attr-defined]
    try:
        engine = QueryEngine(
            llm=args.llm,  # type: ignore[attr-defined]
            database=args.database,  # type: ignore[attr-defined]
        ...
        print(f"[aqlix] Connection failed: {exc}")
        sys.exit(1)

    engine.ingest_schema()
    print("[aqlix] Schema ready.\n")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="aqlix",
        description="aqlix — Natural Language to SQL",
    )
